...
function Import-StsDataFile {
    param([Parameter(Mandatory)][string]$Path,[Parameter(Mandatory)]$DefaultObject)
...
function Export-StsThresholdsFile {
    param([Parameter(Mandatory)][hashtable]$Thresholds,[Parameter(Mandatory)][string]$Path)
...
function Export-StsIgnoreRulesFile {
    param([Parameter(Mandatory)][AllowEmptyCollection()][object[]]$Rules,[Parameter(Mandatory)][string]$Path)
    Ensure-StsParentFolder -Path $Path
    $lines = New-Object System.Collections.Generic.List[string]
    $lines.Add('@{')
    $lines.Add('    Rules = @(')
...
function Show-StsThresholds {
    ...
    'Thresholds'
    '----------'
    foreach ($key in $Thresholds.Keys) { "{0,-28} {1}" -f $key, $Thresholds[$key] }
}
...
function Show-StsIgnoreRules {
    ...
    'Ignore Rules'
    '------------'
    if (@($Rules).Count -eq 0) { '[none]'; return }
...
function Invoke-StsInteractiveThresholdEditor {
    ...
    while ($true) {
        ...
        switch ($name.ToUpperInvariant()) {
            'BACK' { return }
            'LIST' { continue }
            'SAVE' { Export-StsThresholdsFile -Thresholds $Thresholds -Path $Path; "Saved thresholds to $Path"; continue }
            default {
                if (-not $Thresholds.Contains($name)) { "Unknown threshold: $name"; continue }
                $newValueText = Read-Host "New value for $name"
                [double]$newValue = 0
                if (-not [double]::TryParse($newValueText, [ref]$newValue)) { 'Invalid number.'; continue }
...
function Invoke-StsInteractiveIgnoreEditor {
    param([Parameter(Mandatory)][System.Collections.ArrayList]$Rules,[Parameter(Mandatory)][string]$Path)
    while ($true) {
        ''
        ...
        switch ($cmd.ToUpperInvariant()) {
            'BACK' { return }
            'SAVE' { Export-StsIgnoreRulesFile -Rules @($Rules) -Path $Path; "Saved ignore rules to $Path" }
            'REMOVE' {
                if ($Rules.Count -eq 0) { 'There are no rules to remove.'; continue }
                $idxText = Read-Host 'Index to remove'
                [int]$idx = -1
                if (-not [int]::TryParse($idxText, [ref]$idx)) { 'Invalid index.'; continue }
                if ($idx -lt 0 -or $idx -ge $Rules.Count) { 'Index out of range.'; continue }
                $Rules.RemoveAt($idx)
                'Rule removed.'
            }
            'ADD' {
...
if ($thresholdData -and $thresholdData.ContainsKey('Thresholds')) {
    foreach ($key in $thresholdData.Thresholds.Keys) { $thresholds[$key] = $thresholdData.Thresholds[$key] }
}

$rulesList = New-Object System.Collections.ArrayList
foreach ($rule in @($ignoreData.Rules)) { [void]$rulesList.Add([pscustomobject]$rule) }
...
switch ($PSCmdlet.ParameterSetName) {
    ...
    'Ignore' {
        $rule = [pscustomobject]@{
            CheckId=$CheckId; TargetName=$TargetName; Collector=$Collector; State=$State; MatchText=$MatchText; Reason=$Reason;
            Owner=(if ([string]::IsNullOrWhiteSpace($Owner)) { $env:USERNAME } else { $Owner }); Expires=$Expires
        }
    ...
    'RemoveIgnore' {
        if ($IgnoreIndex -lt 0 -or $IgnoreIndex -ge $rulesList.Count) { throw 'IgnoreIndex is out of range.' }
    ...
    default {
        while ($true) {
            ...
            switch ($choice) {
                ...
                '3' { '' ; Show-StsThresholds -Thresholds $thresholds ; '' ; Show-StsIgnoreRules -Rules @($rulesList) }
                '4' { Export-StsThresholdsFile -Thresholds $thresholds -Path $ThresholdsPath; Export-StsIgnoreRulesFile -Rules @($rulesList) -Path $IgnoreRulesPath; 'Saved both config files.' }
                '5' { break }
